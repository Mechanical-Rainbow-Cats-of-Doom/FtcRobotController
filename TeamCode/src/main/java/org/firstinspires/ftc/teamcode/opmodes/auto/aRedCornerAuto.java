...
import static org.firstinspires.ftc.teamcode.roadrunner.util.MirroringUtil.cMirrorX;
...
@Autonomous
public class aRedCornerAuto extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        final Pose2d startPose = cMirrorX(new Pose2d(-32, 63, startingHeading), mirror);

        SampleMecanumDrive drive = new SampleMecanumDrive(hardwareMap);
        drive.setPoseEstimate(startPose);

        final Pose2d middlePosition = cMirrorX(new Pose2d(-35, 9.4, startingHeading), mirror);

        TrajectorySequence start = drive.trajectorySequenceBuilder(startPose)
                .strafeTo(cMirrorX(new Vector2d(-35, 63), mirror))
                .lineToLinearHeading(middlePosition)
                .build();

        final Pose2d dropPosition = cMirrorX(new Pose2d(-23.3, 10, startingHeading), mirror);
        ...
        final Pose2d intakePosition = cMirrorX(new Pose2d(-59.8, 12, startingHeading), mirror);

        TrajectorySequence intake = drive.trajectorySequenceBuilder(dropPosition)
                .strafeTo(cMirrorX(new Vector2d(-22,13), mirror))
                .strafeTo(cMirrorX(new Vector2d(-59.5, 13), mirror))
                .lineToLinearHeading(intakePosition)
                .build();

        TrajectorySequence intakeToMiddle = drive.trajectorySequenceBuilder(cMirrorX(new Pose2d(-57, 12, startingHeading), mirror))
                .strafeTo(cMirrorX(new Vector2d(-35, 12), mirror))
                .lineToLinearHeading(middlePosition)
                .build();

        final Pose2d park0Position = cMirrorX(new Pose2d(-11, 13, startingHeading), mirror);
        final Pose2d park1Position = cMirrorX(new Pose2d(-35, 13, startingHeading), mirror);
        final Pose2d park2Position = cMirrorX(new Pose2d(-57, 13, startingHeading), mirror);
        ...
        turret.setPos(-121, AutoTurret.Units.DEGREES);
        wait(drive, 200, dropPosition);
