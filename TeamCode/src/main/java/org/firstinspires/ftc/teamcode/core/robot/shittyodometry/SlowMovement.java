...
public class SlowMovement {
    ...
    public SlowMovement(HardwareMap hardwareMap, EventThread eventThread, GamepadEx gamepad, AutoLift lift, boolean red) {
    ...
    public void update() {
        ...
        if (moveGamepad.isDown(GamepadKeys.Button.X)) {
            ...
            firstTimeAuto = true;
            drive.setWeightedDrivePower(new Pose2d(
                    moveGamepad.getLeftY() * 0.6,
                    -moveGamepad.getLeftX() * 0.6,
                    moveGamepad.getRightX() * 0.6
            ));
        } else {
            if (firstTimeAuto) {
                zeroMotors();
